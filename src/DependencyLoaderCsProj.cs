    ...
    public static class DependencyLoaderCsProj
    {
        ...
        public static void LoadDependenciesFromNuspec(string nuspecFilePath)
        {
            ...
            foreach (var dependency in dependencies)
            {
                ...
                // Define the path to the .nuspec file of the dependency
                var dependencyNuspecFilePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".nuget", "packages", id.ToLower(), version, $"{id.ToLower()}.nuspec");

                // If the dependency has a .nuspec file, load its dependencies as well
                if (File.Exists(dependencyNuspecFilePath))
                {
                    LoadDependenciesFromNuspec(dependencyNuspecFilePath);
                }
            }
        }
        ...
        private static void LoadAssemblyForPackage(string packageId, string version)
        {
            ...
            for (int i = startIndex; i < allFrameworks.Length; i++)
            {
                ...
                if (File.Exists(assemblyFilePath))
                {
                    try
                    {
                        // If assembly is already loaded, no need to continue
